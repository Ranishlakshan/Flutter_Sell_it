...
class industryToolForm extends StatefulWidget {
  final GlobalKey<ScaffoldState> globalKey;
  const industryToolForm({Key key, this.globalKey}) : super(key: key);
  @override
  _industryToolFormState createState() => _industryToolFormState();
}

class _industryToolFormState extends State<industryToolForm> {
  
  final _formKeyIndustryTool = GlobalKey<FormState>();
      ...
      postImage(imageFile).then((downloadUrl) {
        ...
        if(imageUrls.length==images.length){
          ...
          Firestore.instance.collection('ads').document(documnetID).setData({
            ...
            'reviewstatus':false,  
            'searchkey':itemname+"Machines,Industry Tools tools",
            'value1':itemname,
            'value2':price,
            'value3':testLocation,
            'value4':DateTime.now().toString().substring(0, DateTime.now().toString().length - 10 ),
            //
            'category':"industrytools,BusinessIndustry",
          ...
          RaisedButton(
              ...
              onPressed: () {
                String loccc = '${loccz.getdistrictName()}';
                 if(images.length==0 || !_formKeyIndustryTool.currentState.validate() || loccc=='null' ){
